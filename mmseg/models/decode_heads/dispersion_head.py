...
from ..segmentors.dispersion_encoder_decoder import crop
...
class DispHead(BaseDecodeHead):
    ...
    def __init__(self,
        ...
        if single_scale_head == 'DispDAFormerHead':
        ...
        super(DispHead, self).__init__(**kwargs)
        del self.conv_seg
        del self.dropout
    ...
    def decode_hr(self, inp, bs):
        if isinstance(inp, dict) and 'boxes' in inp.keys():
            ...
            crop_seg_logits = self.head(features)['S']
            for i in range(len(boxes)):
                y1, y2, x1, x2 = boxes[i]
    ...
    def forward(self, inputs):
        ...
        results = self.head(lr_inp)
        lr_seg = results['S']
        lr_seg_init = results['S_init']
        lr_disp = results['D']
        lr_disp_init = results['D_init']
        ...
        up_lr_seg_init = self.resize(lr_seg_init, hr_scale / lr_scale)
        up_lr_disp = self.resize(lr_disp, hr_scale / lr_scale)
        up_lr_disp_init = self.resize(lr_disp_init, hr_scale / lr_scale)
        ...
        return fused_seg, lr_seg, hr_seg, up_lr_disp, up_lr_seg_init, up_lr_disp_init
    ...
    def losses(self, seg_logit, seg_label,
               ...
               seg_weight=None):
        """Compute losses."""
        fused_seg, lr_seg, hr_seg, lr_disp, lr_seg_init, lr_disp_init = seg_logit
        loss = super(DispHead, self).losses(fused_seg, seg_label,
                                            lr_disp, dispersion_map,
                                            seg_weight=seg_weight)
        loss.update(
            add_prefix(
                super(DispHead, self).losses(lr_seg_init, seg_label,
                                             lr_disp_init, dispersion_map,
        ...
        if self.lr_loss_weight > 0:
            loss.update(
                add_prefix(
                    super(DispHead, self).losses(lr_seg, seg_label,
                                                 seg_weight), 'lr'))
        if self.hr_loss_weight > 0 and self.enable_hr_crop:
            ...
            loss.update(
                add_prefix(
                    super(DispHead, self).losses(hr_seg, cropped_seg_label,
                                                 seg_weight=cropped_seg_weight), 'hr'))
        elif self.hr_loss_weight > 0:
            loss.update(
                add_prefix(
                    super(DispHead, self).losses(hr_seg, seg_label,
                                                 seg_weight), 'hr'))
