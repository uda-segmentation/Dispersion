...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    Args:
        ...
        in_index (int|Sequence[int]): Input dispersion_extractor index. Default: -1
        input_transform (str|None): Transformation type of input features.
            Options: 'resize_concat', 'multiple_select', None.
            'resize_concat': Multiple dispersion_extractor maps will be resize to the
                same size as first one and than concat together.
                Usually used in FCN head of HRNet.
            'multiple_select': Multiple dispersion_extractor maps will be bundle into
                a list and passed into decode head.
            None: Only one select dispersion_extractor map is allowed.
            Default: None.
    ...
    def _init_inputs(self, in_channels, in_index, input_transform):
        ...
        Specifically, when input_transform is None, only single dispersion_extractor map
        will be selected. So in_channels and in_index must be of type int.
        When input_transform

        Args:
            in_channels (int|Sequence[int]): Input channels.
            in_index (int|Sequence[int]): Input dispersion_extractor index.
            input_transform (str|None): Transformation type of input features.
                Options: 'resize_concat', 'multiple_select', None.
                'resize_concat': Multiple dispersion_extractor maps will be resize to the
                    same size as first one and than concat together.
    ...
    def losses(self, seg_logit, seg_label,
        ...
        if dispersion_map is not None:
            disp_logit = resize(
                input=disp_logit,
                size=dispersion_map.shape[2:],
                mode='bilinear',
                align_corners=self.align_corners)

            loss['loss_disp'] = self.loss_decode[1](
                disp_logit, dispersion_map
            ) * 0.005
