...
@PIPELINES.register_module()
class LoadDepthMap(object):
    """Load annotations for depth map.
    ...
    Args:
        ...
        to_float32 (bool): Whether to cast the loaded depth map to float32. Default: True.
    """
    def __init__(self,
                 file_client_args=dict(backend='disk'),
                 imdecode_backend='pillow',
    ...
    def __call__(self, results):
        """Call function to load depth map.
        ...
        depth_img = np.exp(-6 * (depth_img / 256)) * 255
        depth_img = 255 - depth_img

        results['depth_img'] = depth_img
        results['depth_fields'].append('depth_img')
        return results
